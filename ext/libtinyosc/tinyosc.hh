...
// round to the next multiple of 4, works for size_t and pointer arguments
	template < typename Type > Type ceil4(Type p) {
		return (Type) ((size_t(p) + 3) & (~size_t(3)));
	}
...
#define OSCPKT_SET_ERR(errcode) do { if (!err) err = errcode; } while (0)
	...
	typedef enum { OK_NO_ERROR = 0,
		...
		MALFORMED_ADDRESS_PATTERN, MALFORMED_TYPE_TAGS,
			MALFORMED_ARGUMENTS, UNHANDLED_TYPE_TAGS,
		// errors raised by ArgReader
		TYPE_MISMATCH, NOT_ENOUGH_ARG, PATTERN_MISMATCH,
		// errors raised by PacketReader/PacketWriter
		INVALID_BUNDLE, INVALID_PACKET_SIZE,
...
*/
  ...
  /** ArgReader is used for popping arguments from a Message, holds a
      pointer to the original Message, and maintains a local error code */
		...
		public:
			ArgReader(const Message & m, ErrorCode e = OK_NO_ERROR):msg(&m),
				...
				err(other.err), arg_idx(other.arg_idx) {
			}
			bool isBool() {
				return currentTypeTag() == TYPE_TAG_TRUE
					|| currentTypeTag() == TYPE_TAG_FALSE;
			}
			bool isInt32() {
				return currentTypeTag() == TYPE_TAG_INT32;
			}
			bool isInt64() {
				return currentTypeTag() == TYPE_TAG_INT64;
			}
			bool isFloat() {
				return currentTypeTag() == TYPE_TAG_FLOAT;
			}
			bool isDouble() {
				return currentTypeTag() == TYPE_TAG_DOUBLE;
			}
			bool isStr() {
				return currentTypeTag() == TYPE_TAG_STRING;
			}
			bool isBlob() {
				return currentTypeTag() == TYPE_TAG_BLOB;
			}

			size_t nbArgRemaining() const {
				return msg->arguments.size() - arg_idx;
			} bool isOk() const {
	...
	/** retrieve an int32 argument */
				ArgReader & popInt32(int32_t & i) {
				return popPod < int32_t > (TYPE_TAG_INT32, i);
			}
	/** retrieve an int64 argument */
			ArgReader & popInt64(int64_t & i) {
				return popPod < int64_t > (TYPE_TAG_INT64, i);
			}
	/** retrieve a single precision floating point argument */
			ArgReader & popFloat(float &f) {
				return popPod < float >(TYPE_TAG_FLOAT, f);
			}
  ...
  */
  ...
  /** build the osc message for raw data (the message will keep a copy of that data) */
		...
		Message & pushBool(bool b) {
			...
			arguments.
				push_back(std::make_pair(storage.size(), storage.size()));
			return *this;
		}
		Message & pushInt32(int32_t i) {
			return pushPod(TYPE_TAG_INT32, i);
		}
		Message & pushInt64(int64_t h) {
			return pushPod(TYPE_TAG_INT64, h);
		}
		Message & pushFloat(float f) {
			return pushPod(TYPE_TAG_FLOAT, f);
		}
		Message & pushDouble(double d) {
			return pushPod(TYPE_TAG_DOUBLE, d);
		}
		Message & pushStr(const std::string & s) {
			assert(s.size() < 2147483647);	// insane values are not welcome
			type_tags += TYPE_TAG_STRING;
			arguments.
				push_back(std::make_pair(storage.size(), s.size() + 1));
  ...
  /** write the raw message data (used by PacketWriter) */
	...
	private:
		...
		template < typename POD > Message & pushPod(int tag, POD v) {
			type_tags += (char) tag;
			arguments.
				push_back(std::make_pair(storage.size(), sizeof(POD)));
...
*/
  ...
  /** pointer and size of the osc packet to be parsed. */
			PacketReader(const void *ptr, size_t sz) {
			init(ptr, sz);
		}
...
*/
  ...
  /** begin a new bundle. If you plan to pack more than one message in the Osc packet, you have to 
      put them in a bundle. Nested bundles inside bundles are also allowed. */
		PacketWriter & startBundle(TimeTag ts = TimeTag::immediate()) {
			char *p;
			if (bundles.size())
				p = storage.getBytes(4);	// hold the bundle size
  ...
  /** return the number of bytes of the osc packet -- will always be a
      ...
      failed. */
		uint32_t packetSize() {
			return err ? 0 : (uint32_t) storage.size();
		}
...
// see the OSC spec for the precise pattern matching rules
	inline const char *internalPatternMatch(const char *pattern,
											const char *path) {
		while (*pattern) {
