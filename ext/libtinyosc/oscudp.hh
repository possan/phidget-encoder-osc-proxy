  ...
  /** retrieve the current port number, -1 in case of error */
		...
		friend std::ostream & operator<<(std::ostream & os,
			...
			case AF_INET6:
				...
				break;
			default:
				os << "unknown family '" << ip.addr().sa_family << "'";
				break;
			}
			os << " " << ip.asString() << "]";
			return os;
...
*/
  ...
  */
...
#ifdef WIN32
				if (res == -1 && WSAGetLastError() == WSAEINTR)
					continue;
				else
					sent = res;
#else
	...
	private:
		...
		bool openSocket(const std::string & hostname,
						const std::string & port, int options) {
			...
			for (rp = result; rp && handle == -1; rp = rp->ai_next) {
				...
				if (binding) {
					if (bind(handle, rp->ai_addr,
						...
						if (getsockname(handle, &local_addr.addr(), &len)
							== 0) {
							/* great */
						}
						break;
