...
void
hexdump(std::ostream & os, const void *s_void, size_t sz, size_t offset)
{
	unsigned char *
		s = (unsigned char *) s_void;
	size_t
		nb = 16;
...
basicTests()
{
	...
	PacketReader
	pr(wr.packetData(), wr.packetSize());
	cerr << "err:" << pr.getErr() << "\n";
	assert(pr.isOk());
	Message *
		mr = pr.popMessage();
	...
	cout << "message received: " << *mr << "\n";
	{
		...
		float
			f1,
			f2;
		if (!mr->arg().popInt32(i1).popInt32(i2).popStr(s).popFloat(f1).
			popFloat(f2).isOk())
			assert(0);
		cerr << "i1=" << i1 << ", i2=" << i2 << ", s='" << s.
			size() << "'\n";
		assert(i1 == 1000);
		assert(i2 == -1);
		assert(s == "hello");
	...
	while (nb_item != 0 && bundle_recurs > 0) {
		if (prandom(7) == 0) {
			...
			for (int i = 0; i < nb_arg; ++i) {
				switch (prandom(8)) {
				case 0:
					msg.pushBool(prandom(2));
					break;
				case 1:
					msg.pushInt32(11223344);
					break;
				case 2:
					msg.pushInt64(123456789012345ll);
					break;
				case 3:
					msg.pushFloat(0.123f);
					break;
				case 4:
					msg.pushDouble(3.14159265358979323846);
					break;
				case 5:{
	...
	while ((msg = pr.popMessage())) {
		...
		while (arg.nbArgRemaining()) {
			if (arg.isBlob()) {
				...
				arg.popBlob(b);
				if (b.size() > 1) {
					check(b.front() == 0x44 || fuzz);
					check(b.back() == 0x66 || fuzz);
				}
			} else if (arg.isBool()) {
				bool b;
				arg.popBool(b);
				...
				int i;
				arg.popInt32(i);
				check(i == 11223344 || fuzz);
			} else if (arg.isInt64()) {
				int64_t h;
				arg.popInt64(h);
				check(h == 123456789012345ll || fuzz);
			} else if (arg.isFloat()) {
				float f;
				arg.popFloat(f);
				check(f == 0.123f || fuzz);
				...
				double d;
				arg.popDouble(d);
				check(d == 3.14159265358979323846 || fuzz);
			} else if (arg.isStr()) {
				std::string s;
				arg.popStr(s);
				if (s.size() > 1) {
					check((s[0] == '<' && s[s.size() - 1] == '>') || fuzz);
				}
			}
		}
		check(arg.isOk() || fuzz);
...
void
randomTests(int nb_test, bool verbose)
{
	...
	for (int fuzz = 0; fuzz < 4; ++fuzz) {
		for (int i = 0; ok && i < nb_test; ++i) {
			if (!makeRandomTest(i, fuzz, verbose))
				ok = false;
		}
	}
	if (ok)
...
#include "lo/lo.h"
static void
lo_osc_error(int num, const char *msg, const char *path)
{
...
libloTest()
{
	...
	sock2.connectTo(lo_url_get_hostname(url), lo_url_get_port(url));
	if (sock2.isOk()) {
...
socketTests()
{
	...
	sock1.bindTo(0);
	if (!sock1.isOk())
		cerr << "sock1: " << sock1.errorMessage() << "\n";

	sock2.connectTo(sock1.localHostName(), sock1.boundPort());
	if (!sock2.isOk())
		cerr << "sock2: " << sock2.errorMessage() << "\n";
	...
	for (int sz = 8 * 8; sz <= 256 * 1024; sz *= 2) {
		...
		if (sent_ok) {
			...
			cerr << "recv_ok=" << recv_ok <<
				", sock1.receiveNextPacket: size=" << sock1.
				packetSize() << " from: " << sock1.packetOrigin();
			if (recv_ok) {
	...
	bool ok;
	ok = sock1.sendPacketTo("plop", 5, sock1.packetOrigin());
	assert(ok);
	cerr << "sock2 is receiving the reply..\n";
	ok = sock2.receiveNextPacket(2000);
	assert(ok);
	cerr << "sock2 has received the reply, sz=" << sock2.
		packetSize() << ": '" << ((char *) sock2.packetData()) << "'\n";
...
checkMatch(const char *pattern, const char *test, bool expected_match = true)
{
	...
	if (!expected_match) {
		if (m) {
			cerr << "unexpected match... " << pattern << " with " << test
				<< "\n";
			assert(0);
		}
	} else {
